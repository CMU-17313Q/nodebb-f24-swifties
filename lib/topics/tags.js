...
module.exports = function (Topics) {
  ...
  function generateTopicSets(tags, cid) {
    console.log('dcharkvi');
    return tags.map(tag => `tag:${tag}:topics`).concat(tags.map(tag => `cid:${cid}:tag:${tag}:topics`));
  }
  ...
  async function getValidationData(tags, cid, uid, tid) {
    return await Promise.all([categories.getCategoryFields(cid, ['minTags', 'maxTags']), user.isPrivileged(uid), tid ? Topics.getTopicTags(tid) : []]);
  }
