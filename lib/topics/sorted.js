...
module.exports = function (Topics) {
  ...
  async function filterTids(tids, params) {
    ...
    const [ignoredCids, filtered] = await Promise.all([getIgnoredCids(params, topicCids), user.blocks.filter(params.uid, topicData)]);
  ...
  function filterTopicsByCidsAndTags(topicData, ignoredCids, params) {
    ...
    const tids = topicData.filter(t => t && t.cid && !isCidIgnored[t.cid] && (!cids || cids.includes(String(t.cid))) && (!tags.length || tags.every(tag => t.tags.find(topicTag => topicTag.value === tag)))).map(t => t.tid);
    return plugins.hooks.fire('filter:topics.filterSortedTids', {
      tids,
      params
    }).then(result => result.tids);
  }
