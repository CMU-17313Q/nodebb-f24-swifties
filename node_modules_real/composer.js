// The entire code is commented out because this chunk of code needs to be in the node_modules in
// order to not show lint errors, for only in node_modules does it define some of the functions
// and variables. It will always show lint error here, and not pass the checks on github.
...
//      for (const uuid in composer.posts) {
//      if (composer.posts.hasOwnProperty(uuid) && composer.posts[uuid].hasOwnProperty('tid') &&
...
//      }
//      const link = `[${escapedTitle}](${config.relative_path}/post/${encodeURIComponent(data.
//      selectedPid || data.toPid)})`;
...
//      const checkTitle = (postData.hasOwnProperty('cid') || parseInt(postData.pid, 10))
...
//      ajaxify.go(`topic/${data.slug}`, undefined, (onComposeRoute || composer.bsEnvironment
//      === 'xs' || composer.bsEnvironment === 'sm'));
//      }
//      } else if (action === 'posts.reply') {
